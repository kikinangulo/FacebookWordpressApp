<?php
if ( !is_admin() ) { 
	//wp_enqueue_script('jquery.validate', 'https://ajax.aspnetcdn.com/ajax/jquery.validate/1.9/jquery.validate.min.js', array());
	wp_enqueue_script('facebook', 'https://connect.facebook.net/'.FB_LOCALE.'/all.js', array());
	wp_enqueue_script('appjs', get_template_directory_uri() . '/js/app.js', array());
	
	// Disable frontend WP admin bar for logged in users
...
{
	$internalLink = false;
	if(isset($_GET['fb_request'])) {
		$fb_request = object_to_array(json_decode(base64_decode($_GET['fb_request'])));
		define("FBREQ", $_GET['fb_request']);
		$internalLink = true;
	} else {
		...
		$fb_request = $facebook->getSignedRequest();
		define("FBREQ", base64_encode(json_encode($fb_request)) );
	}	
	($fb_request['page']['liked'] == 1) ? $isFan = true : $isFan = false;
	define("FB_PAGE_FAN", $isFan);
	$appData = json_decode($fb_request['app_data'], true);

	// Check if $appData contains "page", if so we'll redirect to it!
	if(!empty($appData['page']) && is_page('frontpage') && !$internalLink){
		//die(WP_HOME . $appData['page'] . ((!strstr($app_data->url, '?'))? '?': '&').'fb_request='. FBREQ);
		wp_redirect(WP_HOME . $appData['page'] . ((!strstr($app_data->url, '?'))? '?': '&').'fb_request='. FBREQ);
		exit;
	}
}
...
function boilerplate_filter_wp_title( $title, $separator ) {
	...
	// If we have a site description and we're on the home/front page, add the descriptionion:
